...
namespace Drupal\cron_orders\Form;
 ...
 */
class CronOrdersForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->configFactory->get('cron_orders.settings');
    ...
    $args = [
      '%time' => date_iso8601(\Drupal::state()->get('cron_orders.next_execution')),
      '%seconds' => $next_execution - REQUEST_TIME,
    ];
    $form['status']['last'] = [
      '#type' => 'item',
      '#markup' => $this->t('cron_orders_cron() will next execute the first time cron runs after %time (%seconds seconds from now)', $args),
    ];
    ...
    if ($this->currentUser->hasPermission('administer site configuration')) {
      ...
      $form['cron_run']['cron_reset'] = [
        '#type' => 'checkbox',
        '#title' => $this->t("Run cron_orders's cron regardless of whether interval has expired."),
        '#default_value' => FALSE,
      ];
    ...
    $queue_1 = $this->queue->get('cron_orders_queue_1');
    $queue_2 = $this->queue->get('cron_orders_queue_2');
    ...
    $form['cron_queue_setup']['queue'] = [
      ...
      '#options' => [
        'cron_orders_queue_1' => $this->t('Queue 1'),
        'cron_orders_queue_2' => $this->t('Queue 2'),
      ],
      '#default_value' => 'cron_orders_queue_1',
    ];
    ...
    $form['configuration'] = [
      '#type' => 'details',
      '#title' => $this->t('Configuration of cron_orders_cron()'),
      '#open' => TRUE,
    ];
    $form['configuration']['cron_orders_interval'] = [
      '#type' => 'select',
      '#title' => $this->t('Cron interval'),
      '#description' => $this->t('Time after which cron_orders_cron will respond to a processing request.'),
  ...
  public function cronRun(array &$form, FormStateInterface &$form_state) {
    $config = $this->configFactory->getEditable('cron_orders.settings');

    $cron_reset = $form_state->getValue('cron_reset');
    if (!empty($cron_reset)) {
      \Drupal::state()->set('cron_orders.next_execution', 0);
    }

    // Use a state variable to signal that cron was run manually from this form.
    $this->state->set('cron_orders_show_status_message', TRUE);
    if ($this->cron->run()) {
  ...
  public function addItems(array &$form, FormStateInterface &$form_state) {
    ...
    // id attritbute.
    // @see \Drupal\cron_orders\Plugin\QueueWorker\ReportWorkerOne
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->configFactory->getEditable('cron_orders.settings')
      ->set('interval', $form_state->getValue('cron_orders_interval'))
      ->save();
