...
namespace Drupal\cron_barrett\Form;
...
/**
 * Form with barretts on how to use cron.
 */
class CronbarrettForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->configFactory->get('cron_barrett.settings');
    ...
    $form['status']['intro'] = [
      '#type' => 'item',
      '#markup' => $this->t('The cron barrett demonstrates hook_cron() and hook_queue_info() processing. If you have administrative privileges you can run cron from this page and see the results.'),
    ];
    ...
    $args = [
      '%time' => date_iso8601(\Drupal::state()->get('cron_barrett.next_execution')),
      '%seconds' => $next_execution - REQUEST_TIME,
    ];
    $form['status']['last'] = [
      '#type' => 'item',
      '#markup' => $this->t('cron_barrett_cron() will next execute the first time cron runs after %time (%seconds seconds from now)', $args),
    ];
    ...
    if ($this->currentUser->hasPermission('administer site configuration')) {
      ...
      $form['cron_run']['cron_reset'] = [
        '#type' => 'checkbox',
        '#title' => $this->t("Run cron_barrett's cron regardless of whether interval has expired."),
        '#default_value' => FALSE,
      ];
    ...
    $queue_1 = $this->queue->get('cron_barrett_queue_1');
    $queue_2 = $this->queue->get('cron_barrett_queue_2');
    ...
    $form['cron_queue_setup']['queue'] = [
      ...
      '#options' => [
        'cron_barrett_queue_1' => $this->t('Queue 1'),
        'cron_barrett_queue_2' => $this->t('Queue 2'),
      ],
      '#default_value' => 'cron_barrett_queue_1',
    ];
    ...
    $form['configuration'] = [
      '#type' => 'details',
      '#title' => $this->t('Configuration of cron_barrett_cron()'),
      '#open' => TRUE,
    ];
    $form['configuration']['cron_barrett_interval'] = [
      '#type' => 'select',
      '#title' => $this->t('Cron interval'),
      '#description' => $this->t('Time after which cron_barrett_cron will respond to a processing request.'),
    ...
    $form['configurationenv'] = [
      '#type' => 'details',
      '#title' => $this->t('Configuration of Barrett environment: (PROD or TEST)'),
      '#open' => TRUE,
    ];
    $form['configurationenv']['cron_barrett_envbarrett'] = [
      '#type' => 'select',
      '#title' => $this->t('Cron Environment'),
      '#description' => $this->t('Environment where barrett have to run.'),
      '#default_value' => $config->get('envbarrett'),
      '#options' => [
        0 => $this->t('Staging'),
        1 => $this->t('Production')
      ],
    ];
    $form['configurationenv']['cron_barrett_contactusemail'] = [
      '#type' => 'select',
      '#title' => $this->t('Contact mail to sent'),
      '#description' => $this->t('Prod email is info@celebluxury.com.'),
      '#default_value' => $config->get('contactusemail'),
      '#options' => [
        "bigbabert@gmail.com" => $this->t('Staging'),
        "info@celebluxury.com" => $this->t('Production')
      ],
    ];
  ...
  public function cronRun(array &$form, FormStateInterface &$form_state) {
    $config = $this->configFactory->getEditable('cron_barrett.settings');

    $cron_reset = $form_state->getValue('cron_reset');
    if (!empty($cron_reset)) {
      \Drupal::state()->set('cron_barrett.next_execution', 0);
    }

    // Use a state variable to signal that cron was run manually from this form.
    $this->state->set('cron_barrett_show_status_message', TRUE);
    if ($this->cron->run()) {
  ...
  public function addItems(array &$form, FormStateInterface &$form_state) {
    ...
    // id attritbute.
    // @see \Drupal\cron_barrett\Plugin\QueueWorker\ReportWorkerOne
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->configFactory->getEditable('cron_barrett.settings')
        ->set('interval', $form_state->getValue('cron_barrett_interval')) 
        ->set('envbarrett', $form_state->getValue('cron_barrett_envbarrett')) 
        ->set('contactusemail', $form_state->getValue('cron_barrett_contactusemail')) 
      ->save();
